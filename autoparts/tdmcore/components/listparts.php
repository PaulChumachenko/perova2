...
if (0 < count($arPARTS_noP)) {
	...
	foreach ($arSrCrParts as $PKEY => $arTPart) {
		...
		while ($arLink = $rsLinks->Fetch()) {
			...
			if ($arLink["SIDE"] == 1) {
				$SLable = "&#8594;";
			} elseif ($arLink["SIDE"] == 2) {
				$SLable = "&#8592;";
			} else {
				$SLable = "&#8596;";
			}

			$arPARTS_noP[$arLink['PKEY2']] = [
				'PKEY'      => $arLink['PKEY2'],
				'BKEY'      => $arLink['BKEY2'],
				'BRAND'     => $arLink['BKEY2'],
				'AKEY'      => $arLink['AKEY2'],
				'ARTICLE'   => $arLink['AKEY2'],
				'LINK_SIDE' => $arLink['SIDE'],
				'LINK_CODE' => $arLink['CODE'],
				'LINK_INFO' => '<b>' . $arLink['BKEY1'] . '</b> ' . $arLink['AKEY1'] . ' ' . $SLable . ' <b>' . $arLink['BKEY2'] . '</b> ' . $arLink['AKEY2'],
	...
	// PC: Load items from External APIs
