...
// PC: Initialize Alternative Articles Manager
		...
		if ($arCFile = file($arSup["FILE_PATH"])) {
			...
			if ($arSup["START_FROM"] < $CSVcount) {
				...
				foreach ($arCFile as $Line => $strLine) {
					...
					// PC: Flush previous Alternative Articles
					if ($lastProcessedLine == 1) $altArticles->flushPrevious();
					...
					$arFields["ARTICLE"] = $arFields["ARTICLE"] ? $arFields["ARTICLE"] : PcHelper::getCustomKey($arCSVrow[7], $arSup);
					$arFields["BRAND"] = $arFields["BRAND"] ? $arFields["BRAND"] : PcHelper::getCustomKey($arCSVrow[9], $arSup);
					...
					if ($arFields["BKEY"] != "" && $arFields["AKEY"] != "" && 0 < $arFields["PRICE"]) {
						...
						// PC: Import Alternative Articles for current Prices Row
				...
				// Start crossing process after successful import
				if($lastProcessedLine == $CSVcount && $_GET["TEST"] != "Y"){
					$cg = new PcCrossGenerator();
					$cg->run();
					$JSCrossGenInfo = "<script>alert('Кроссы успешно созданы. Затрачено времени: {$cg->getExecutionTime()} сек');</script>";
				}
			}
			else {
